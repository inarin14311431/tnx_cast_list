...
(function(){
  ...
  function outfitCost(finalMundane){
    let total=0;
    const seen=new Set();
    for(const control of $$('#outfit-list [data-o="experience_cost"]')){
      ...
      seen.add(key);
      const permanent=Math.max(0,num(control.value));
      total+=Math.max(0,permanent-finalMundane);
    }
    return total;
  }
  ...
  function calculate(){
    ...
    const finalMundane=num($("#mundane-final")?.textContent||num($("#mundane-base")?.value)+num($("#mundane-mod")?.value));
    const outfit=outfitCost(finalMundane);

    // Character-construction allowances are applied internally and are not shown as rows.
    const gross=ability+control+skills.general+skills.style+outfit;
    const total=Math.max(0,gross-170-20-15);
    ...
    if(breakdown){
      ...
      breakdown.innerHTML=Object.entries(parts).map(([label,value])=>`<div><dt>${label}</dt><dd>${value}</dd></div>`).join("");
    }
  }
