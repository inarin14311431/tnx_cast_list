...
(()=>{
  ...
  function selectBuiltIn(master,allRows,used){
    ...
    /* Saved rows are rendered before the generated bare 0Lv placeholder.
     * Therefore the first specialized row is the persistent built-in slot. */
  ...
  function applyOrder(body,desired){
    ...
    desired.forEach((row,index)=>{
      const current=[...body.rows].filter(item=>!item.hidden)[index];
  ...
  function arrange(){
    ...
    try{
      ...
      const used=new Set();
      const left=desiredRows(firstBody,LEFT_MASTER,allRows,used);
      const right=desiredRows(secondBody,RIGHT_MASTER,allRows,used);
      applyOrder(firstBody,left);
      applyOrder(secondBody,right);
    }finally{
      arranging=false;
