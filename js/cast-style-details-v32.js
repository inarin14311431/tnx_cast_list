...
const FIELDS=[["skill","技能"],["limit","上限"],["timing","タイミング"],["target","対象"],["range","射程"],["difficulty","目標値"],["confrontation","対決"],["description","解説"],["page","参照"]];
const SUIT_HEADERS=["理性","感情","生命","外界"];
...
async function render(){
  ...
  const section=[...container.querySelectorAll(".skill-section")].find(item=>/STYLE SKILLS|スタイル技能/i.test(item.querySelector("h3")?.textContent||""));
  if(!section)return;
  section.classList.add("style-skill-section-v32");
  section.innerHTML=`<h3>スタイル技能 <small>STYLE SKILLS</small></h3><div class="data-table-wrapper"><table class="data-table style-skill-detail-table"><thead><tr><th>名称</th><th>種別</th><th>LV</th>${SUIT_HEADERS.map(label=>`<th>${label}</th>`).join("")}${FIELDS.map(([,label])=>`<th>${label}</th>`).join("")}</tr></thead><tbody>${skills.map(skill=>{const detail=parse(skill.description);const kind={normal:"通常",secret:"秘技",ultimate:"奥義"}[skill.skill_kind]||skill.skill_kind||"";return `<tr><td>${esc(skill.name)}</td><td>${esc(kind)}</td><td>${esc(skill.level)}</td><td>${skill.reason?"●":""}</td><td>${skill.passion?"●":""}</td><td>${skill.life?"●":""}</td><td>${skill.mundane?"●":""}</td>${FIELDS.map(([key])=>`<td>${esc(detail[key]).replaceAll("\n","<br>")}</td>`).join("")}</tr>`;}).join("")}</tbody></table></div>`;
}
const root=document.querySelector("#cast-content");
if(root){
  const observer=new MutationObserver(()=>{if(!root.hidden){observer.disconnect();render();}});
