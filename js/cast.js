...
function renderQuickSheet(context) {
  const { character, combos } = context;
  const usageLimits = new Map(combos.map(combo => [String(combo.id), getComboActUseLimit(combo)]).filter(([, limit]) => limit !== null));
  const usageState = combos.length ? loadComboUsageState(getComboUsageStorageKey(character), usageLimits) : new Map();
  quickSheetPages.innerHTML = createQuickSheetHtml(context, usageState);
