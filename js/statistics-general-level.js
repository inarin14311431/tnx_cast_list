...
async function renderLevelWeightedGeneralSkills() {
  try {
    ...
    for (const skill of skills ?? []) {
      ...
      if (!name) continue;
      incrementBy(generalSkillLevels, name, Math.max(0, numberOrZero(skill.level)));
    }
...
function incrementBy(map, key, value) { map.set(key, (map.get(key) ?? 0) + value); }
function numberOrZero(value) { const number = Number(value); return Number.isFinite(number) ? number : 0; }
function formatNumber(value) { return Number(value).toLocaleString("ja-JP", { maximumFractionDigits: 2 }); }
