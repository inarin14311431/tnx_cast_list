/* Browser draft persistence for the sheet editor. */
(function(){
  ...
  function restoreOutfits(saved){
    ...
    let guard=0;
    while(document.querySelectorAll("#outfit-list [data-outfit-key]").length<meaningful.length&&guard++<50)document.querySelector("#add-outfit")?.click();
    const cards=[...document.querySelectorAll("#outfit-list [data-outfit-key]")];
    meaningful.forEach((fields,index)=>{
      const card=cards[index];if(!card)return;
      const data=rowData(fields);
      for(const [field,value] of Object.entries(data)){
        const element=card.querySelector(`[data-o="${field}"]`);
        if(element)assign(element,value,true);
      }
    });
  }
