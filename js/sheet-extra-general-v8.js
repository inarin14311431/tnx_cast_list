...
function initializeMasterRows(){
  ...
  if(!container)return;
  const observer=new MutationObserver(()=>injectMasterRows());
...
function injectMasterRows(){
  ...
  for(const name of MASTER_PROPER_SKILLS){
    const exists=[...generalTable.querySelectorAll('input[data-f="name"]')]
    ...
    row.querySelector(".master-proper-acquire").addEventListener("click",()=>acquireMasterSkill(name));
...
function acquireMasterSkill(name){
  ...
  requestAnimationFrame(()=>{
    ...
    if(kindSelect){kindSelect.value="proper";kindSelect.dispatchEvent(new Event("input",{bubbles:true}));}
    if(levelInput){levelInput.value="1";levelInput.dispatchEvent(new Event("input",{bubbles:true}));}
  });
}
