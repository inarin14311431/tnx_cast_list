...
let saveTimer, saving = false, pendingSave = false, loading = false, dirty = false, importMode = "";
const styleBaseline = {};
...
async function init(){
  ...
  if(publicId) await loadCharacter(publicId); else createNewState();
}
...
function bindEvents(){
  ...
  document.addEventListener("click", event => {
    ...
    if(deleteSkill){ skills = skills.filter(x => x._key !== deleteSkill.dataset.deleteSkill); renderSkills(); recalc(); markDirty(); return; }
    const deleteOutfit = event.target.closest("[data-delete-outfit]");
    if(deleteOutfit){ outfits = outfits.filter(x => x._key !== deleteOutfit.dataset.deleteOutfit); renderOutfits(); recalc(); markDirty(); }
  });
  $("#save-button").addEventListener("click", () => saveAll(true));
  $("#toggle-general").addEventListener("click", () => { showAllGeneral = !showAllGeneral; updateGeneralToggleLabel(); renderSkills(); });
...
async function loadCharacter(publicId){
  ...
  if(error || !data){ setStatus(toJapaneseError(error?.message || "キャストを読み込めませんでした。"),"error"); loading = false; return; }
  character = data;
  fillCharacter(data);
...
function fillCharacter(data){
  ["character_name","character_kana","handle","player_name","affiliation","citizen_rank","summary","profile","visibility"].forEach(name => { const element = $("#" + name.replaceAll("_","-")); if(element) element.value = data[name] ?? ""; });
  for(let i=1;i<=3;i++){
...
function renderStyles(){
  ...
  const attributes = '<option value="">属性を選択</option>' + UTSUWA_ATTRIBUTES.map(x => `<option>${esc(x.name)}</option>`).join("");
...
function styleRecord(i){
  ...
  if(name === "ウツワ") return UTSUWA_ATTRIBUTES.find(x => x.name === $(`#style-${i}-attribute`).value) || null;
  return STYLE_DATA.find(x => x.name === name) || null;
}
function calculateStyleBaselines(){
  ...
  for(let i=1;i<=3;i++){
    ...
    if(!record) continue;
    for(const [key] of ABILITIES){ styleBaseline[key] += Number(record[key]?.[0] || 0); styleBaseline[`${key}-control`] += Number(record[key]?.[1] || 0); }
  }
}
function updateDivines(applyBaseline=false){
  for(let i=1;i<=3;i++){
    const style = STYLE_DATA.find(x => x.name === $(`#style-${i}`).value);
  ...
  calculateStyleBaselines();
  for(const [key] of ABILITIES){ adjustCurrentForBaseline(key,previous[key] || 0,styleBaseline[key] || 0); adjustCurrentForBaseline(`${key}-control`,previous[`${key}-control`] || 0,styleBaseline[`${key}-control`] || 0); }
  recalc();
}
...
function renderAbilities(){ $("#ability-grid").innerHTML = ABILITIES.map(([key,jp,en]) => abilityCard(key,jp,en)).join("") + csCard(); }
function abilityCard(key,jp,en){
  return `<article class="ability-card ability-matrix"><h3>${jp} <small>${en}</small></h3><div class="ability-matrix__header"><span></span><strong>能力値</strong><strong>制御値</strong></div><div class="ability-matrix__row"><span>現在値</span><input id="${key}-base" type="number" min="0" value="0"><input id="${key}-control-base" type="number" min="0" value="0"></div><div class="ability-matrix__row"><span>補正値</span><input id="${key}-mod" type="number" value="0"><input id="${key}-control-mod" type="number" value="0"></div><div class="ability-matrix__row ability-matrix__result"><span>最終値</span><strong id="${key}-final">0</strong><strong id="${key}-control-final">0</strong></div></article>`;
}
function csCard(){ return `<article class="ability-card ability-card--cs"><h3>CS</h3><div class="cs-row"><label>現在値<input id="cs-base" type="number" value="0"></label><label>補正値<input id="cs-mod" type="number" value="0"></label><strong id="cs-final">0</strong></div></article>`; }
...
function normalizeSkill(skill){ const result={...blankSkill(skill.category),...skill,_key:skill.id || crypto.randomUUID(),free_level:Number(skill.free_level || 0),skill_kind:skill.skill_kind || inferSkillKind(skill)}; if(result.name === "初期取得") result.name = result.category === "connection" ? "コネ：初期取得" : "社会：初期取得"; return result; }
function inferSkillKind(skill){ if(skill.category === "style") return /奥義/.test(skill.type || "") ? "ultimate" : /秘技/.test(skill.type || "") ? "secret" : "normal"; return String(skill.name || "").includes("：") ? "proper" : "general"; }
function updateGeneralToggleLabel(){ const button=$("#toggle-general"); if(button) button.innerHTML = showAllGeneral ? "取得済みのみ表示 <small>OWNED ONLY</small>" : "全技能表示 <small>SHOW ALL</small>"; }
function renderSkills(){
  const merged = mergeGeneralMaster(skills);
  const generalRows = showAllGeneral ? merged.filter(x => x.category === "general") : merged.filter(x => x.category === "general" && Number(x.level)>0);
  $("#general-skills").innerHTML = [skillTable("一般技能","GENERAL SKILLS",generalRows,false),skillTable("社会","SOCIAL",skills.filter(x=>x.category === "social"),false),skillTable("コネクション","CONNECTIONS",skills.filter(x=>x.category === "connection"),false)].join("");
  $("#style-skills").innerHTML = skillTable("スタイル技能","STYLE SKILLS",skills.filter(x=>x.category === "style"),true);
  bindSkillRows();
}
function mergeGeneralMaster(list){ const output=[...list]; for(const [name] of FIXED_GENERAL) if(!output.some(x=>x.category === "general" && x.name === name)) output.push({...blankSkill("general"),name,level:0}); return output; }
...
function bindSkillRows(){
  $$('[data-skill-key]').forEach(row => row.querySelectorAll('[data-f]').forEach(element => {
    element.oninput = () => {
      ...
      if(SUITS.includes(field)){ skill.level = SUITS.filter(suit => skill[suit]).length; renderSkills(); }
      else if(field === "level"){ const level=Math.max(0,Number(element.value || 0)); skill.level=level; SUITS.forEach((suit,index)=>skill[suit]=index<Math.min(level,4)); renderSkills(); }
      recalc();
      markDirty();
    };
  }));
...
function renderOutfits(){
  $("#outfit-list").innerHTML = outfits.map(outfit=>`<article class="outfit-card" data-outfit-key="${outfit._key}"><label>分類<select data-o="category">${["weapon","armor","cyberware","tron","vehicle","residence","other"].map(value=>`<option value="${value}" ${outfit.category === value ? "selected" : ""}>${{weapon:"武器",armor:"防具",cyberware:"サイバーウェア",tron:"トロン",vehicle:"ヴィークル",residence:"住居",other:"その他"}[value]}</option>`).join("")}</select></label><label>名称<input data-o="name" value="${esc(outfit.name)}"></label><label>購入<input data-o="purchase_value" value="${esc(outfit.purchase_value)}"></label><label>常備化<input data-o="experience_cost" type="number" value="${outfit.experience_cost}"></label><label>部位<input data-o="slot" value="${esc(outfit.slot)}"></label><label>解説<input data-o="description" value="${esc(outfit.description)}"></label><button class="row-delete" data-delete-outfit="${outfit._key}" type="button">×</button></article>`).join("") || "<p>アウトフィット未登録</p>";
  $$('[data-outfit-key]').forEach(card => card.querySelectorAll('[data-o]').forEach(element => { element.oninput = () => { const outfit=outfits.find(x=>x._key === card.dataset.outfitKey); outfit[element.dataset.o] = element.type === "number" ? Number(element.value) : element.value; recalc(); markDirty(); }; }));
}
...
function recalc(){
  for(const [key] of ABILITIES){ $(`#${key}-final`).textContent = finalValue(key); $(`#${key}-control-final`).textContent = finalValue(`${key}-control`); }
  $("#cs-final").textContent = Number($("#cs-base").value || 0) + Number($("#cs-mod").value || 0);
  const exp=calculateExp(), total=$("#exp-total"), previous=Number(total.textContent || 0);
  total.textContent=exp.total;
  if(previous !== exp.total){ total.classList.remove("flash"); void total.offsetWidth; total.classList.add("flash"); }
...
function calculateExp(){
  ...
  for(const [key] of ABILITIES){
    const base=Number(styleBaseline[key] || 0), growth=Math.max(0,currentValue(key)-base);
    for(let i=1;i<=growth;i++) ability += base+i <= 10 ? 20 : 40;
    const controlKey=`${key}-control`, controlBase=Number(styleBaseline[controlKey] || 0), controlGrowth=Math.max(0,currentValue(controlKey)-controlBase);
    for(let i=1;i<=controlGrowth;i++) control += controlBase+i <= 16 ? 20 : 40;
  }
  for(const skill of skills){ const paid=Math.max(0,Number(skill.level || 0)-Number(skill.free_level || 0)); if(skill.category === "style") style += paid*({normal:10,secret:20,ultimate:50}[skill.skill_kind] || 10); else general += paid*(skill.skill_kind === "proper" ? 5 : 10); }
  const mundane=finalValue("mundane");
  for(const item of outfits){ const permanent=Number(item.experience_cost || 0); if(permanent>mundane) outfit += permanent; }
...
async function saveAll(force){
  ...
  try{
    ...
    let operation=await supabase.from("character_skills").delete().eq("character_id",character.id); if(operation.error) throw operation.error;
    operation=await supabase.from("character_outfits").delete().eq("character_id",character.id); if(operation.error) throw operation.error;
    const skillRows=skills.filter(x=>Number(x.level)>0 && x.name.trim()).map((x,index)=>({character_id:character.id,category:x.category,name:x.name,level:Number(x.level || 0),free_level:Number(x.free_level || 0),skill_kind:x.skill_kind,reason:!!x.reason,passion:!!x.passion,life:!!x.life,mundane:!!x.mundane,timing:x.timing || "",target:x.target || "",range:x.range || "",difficulty:x.difficulty || "",confrontation:x.confrontation || "",description:x.description || "",sort_order:index}));
    ...
    const outfitRows=outfits.filter(x=>x.name.trim()).map((x,index)=>({character_id:character.id,category:x.category,name:x.name,purchase_value:x.purchase_value || "",experience_cost:Number(x.experience_cost || 0),concealment:x.concealment || "",attack:x.attack || "",defense:x.defense || "",range:x.range || "",slot:x.slot || "",description:x.description || "",control_modifier:Number(x.control_modifier || 0),cs_modifier:Number(x.cs_modifier || 0),mundane_modifier:Number(x.mundane_modifier || 0),sort_order:index}));
...
function pulseSaveButton(state){ const button=$("#save-button"); button.classList.remove("is-saving","is-saved","is-error"); void button.offsetWidth; button.classList.add(state === "saving" ? "is-saving" : state === "saved" ? "is-saved" : "is-error"); }
