...
function inferKind(skill){ if(skill.category==="style")return window.TNXStyleSkillKinds?.fromLabel(skill.type||skill.kind)||(/演出|方向/.test(skill.type||"")?"direction":/奥義/.test(skill.type||"")?"ultimate":/秘技/.test(skill.type||"")?"secret":/なし/i.test(skill.type||"")?"none":"normal"); return String(skill.name||"").includes("：")?"proper":"general"; }
