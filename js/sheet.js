...
function addSkill(category, kind, name){ skills.push({...blankSkill(category), skill_kind:kind, name}); renderSkills(); markDirty(); }

function createNew(){
  loading = true;
  character = {visibility:"private"};
  $("#visibility").value = "private";
  skills = GENERAL_MASTER.filter(item => item[2] === "general").map(([name,suit]) => ({...blankSkill("general"),name,level:1,free_level:1,[suit]:true,skill_kind:"general"}));
  skills.push(
    {...blankSkill("social"),name:"社会：N◎VA",level:1,free_level:1,skill_kind:"proper"},
    {...blankSkill("social"),name:"社会：初期取得",level:1,free_level:1,skill_kind:"proper"},
    {...blankSkill("connection"),name:"コネ：初期取得",level:1,free_level:1,skill_kind:"proper"},
    {...blankSkill("connection"),name:"コネ：初期取得",level:1,free_level:1,skill_kind:"proper"}
  );
...
function normalizeSkill(skill){ const result={...blankSkill(skill.category),...skill,_key:skill.id||crypto.randomUUID(),free_level:Number(skill.free_level||0),skill_kind:skill.skill_kind||inferKind(skill)}; if(result.name==="初期取得")result.name=result.category==="connection"?"コネ：初期取得":"社会：初期取得"; return result; }
...
function recalc(){ for(const [key] of ABILITIES){ $(`#${key}-final`).textContent=final(key); $(`#${key}-control-final`).textContent=final(`${key}-control`); } $("#cs-final").textContent=Number($("#cs-base").value||0)+Number($("#cs-mod").value||0); const exp=calcExp(),total=$("#exp-total"),old=Number(total.textContent||0); total.textContent=exp.total; if(old!==exp.total){ total.classList.remove("flash"); void total.offsetWidth; total.classList.add("flash"); } $("#exp-breakdown").innerHTML=Object.entries(exp.parts).map(([label,value])=>`<div><dt>${label}</dt><dd>${value}</dd></div>`).join(""); }
function calcExp(){ let ability=0,control=0,general=0,style=0,outfit=0; for(const [key] of ABILITIES){ const base=Number(styleBaseline[key]||0),growth=Math.max(0,current(key)-base); for(let i=1;i<=growth;i++)ability+=base+i<=10?20:40; const controlKey=`${key}-control`,controlBase=Number(styleBaseline[controlKey]||0),controlGrowth=Math.max(0,current(controlKey)-controlBase); for(let i=1;i<=controlGrowth;i++)control+=controlBase+i<=16?20:40; } for(const skill of skills){ const paid=Math.max(0,Number(skill.level||0)-Number(skill.free_level||0)); if(skill.category==="style")style+=paid*({normal:10,secret:20,ultimate:50}[skill.skill_kind]||10); else general+=paid*(skill.skill_kind==="proper"?5:10); } const mundane=final("mundane"); for(const outfitItem of outfits){ const permanent=Number(outfitItem.experience_cost||0); if(permanent>mundane)outfit+=permanent; } return {total:ability+control+general+style+outfit,parts:{能力値:ability,制御値:control,一般技能:general,スタイル技能:style,アウトフィット:outfit}}; }
...
function collect(includeAttribute=true){ const payload={owner_id:user.id,character_name:$("#character-name").value.trim(),character_kana:$("#character-kana").value.trim(),handle:$("#handle").value.trim(),player_name:$("#player-name").value.trim(),affiliation:$("#affiliation").value.trim(),citizen_rank:$("#citizen-rank").value.trim(),summary:$("#summary").value,profile:$("#profile").value,visibility:$("#visibility").value,experience_points:Number($("#exp-total").textContent||0)}; for(let i=1;i<=3;i++){ const style=STYLE_DATA.find(item=>item.name===$(`#style-${i}`).value); payload[`style_${i}`]=$(`#style-${i}`).value; payload[`style_${i}_mark`]=$(`#style-${i}-mark`).value; if(includeAttribute)payload[`style_${i}_attribute`]=$(`#style-${i}-attribute`)?.value||""; payload[`divine_${i}`]=style?.divine||""; payload[`divine_${i}_yomi`]=style?.divineYomi||style?.divine||""; } for(const [key] of ABILITIES){ payload[`${key}_base`]=current(key); payload[`${key}_growth`]=Math.max(0,current(key)-Number(styleBaseline[key]||0)); payload[`${key}_gear`]=Number($(`#${key}-mod`).value||0); payload[`${key}_manual`]=0; payload[`${key}_value`]=final(key); const controlKey=`${key}-control`; payload[`${key}_control_base`]=current(controlKey); payload[`${key}_control_growth`]=Math.max(0,current(controlKey)-Number(styleBaseline[controlKey]||0)); payload[`${key}_control_gear`]=Number($(`#${controlKey}-mod`).value||0); payload[`${key}_control_manual`]=0; payload[`${key}_control`]=final(controlKey); } payload.cs_base=Number($("#cs-base").value||0); payload.cs_gear=Number($("#cs-mod").value||0); payload.cs_manual=0; payload.cs=payload.cs_base+payload.cs_gear; return payload; }
