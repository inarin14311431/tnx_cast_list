...
const VISIBILITY_LABELS = { public: "公開 / PUBLIC", private: "非公開 / PRIVATE" };
...
async function loadOwnedCharacters() {
  ...
  const { data, error } = await supabase.from("characters").select("id, public_id, character_name, character_kana, handle, style_1, style_1_mark, style_2, style_2_mark, style_3, style_3_mark, visibility, updated_at").eq("owner_id", currentUser.id).order("updated_at", { ascending: false });
  if (error) {
    console.error(error);
    ownedCharacters = [];
...
function populateOwnedCastStyles() {
  ...
  const styles = [...new Set(ownedCharacters.flatMap(character => [character.style_1, character.style_2, character.style_3]).filter(Boolean))].sort(localeCompareJa);
...
function renderOwnedCharacters() {
  ...
  const filtered = ownedCharacters.filter(character => {
    ...
    return (!keyword || searchable.includes(keyword)) && (!visibility || character.visibility === visibility) && (!style || styles.includes(style));
  });
  ...
  ownedCastsContainer.querySelectorAll("[data-delete]").forEach(button => button.addEventListener("click", () => deleteCharacter(button.dataset.delete)));
  ownedCastsContainer.querySelectorAll("[data-duplicate]").forEach(button => button.addEventListener("click", () => duplicateCharacter(button.dataset.duplicate)));
}
...
function actionLabel(japanese, english) { return `<span class="action-label__jp">${japanese}</span><small class="action-label__en">${english}</small>`; }
function visibilityLabel(value) { const key = String(value ?? "").toLowerCase(); return VISIBILITY_LABELS[key] ?? VISIBILITY_LABELS.private; }
...
function createOwnedCastItem(character) {
  ...
  const styles = [[character.style_1, character.style_1_mark],[character.style_2, character.style_2_mark],[character.style_3, character.style_3_mark]].filter(([name]) => name).map(([name, mark]) => `
    <span class="owned-cast__style" style="--style-color:${getStyleColor(name)}"><span>${escapeHtml(name)}</span>${mark ? `<b>${escapeHtml(mark)}</b>` : ""}</span>`).join("");
  return `
    <article class="owned-cast">
      <div class="owned-cast__identity"><p class="owned-cast__handle">${escapeHtml(window.TNXHandleFormat?.quoteHandle(character.handle) || "ハンドル未登録")}</p><h3>${escapeHtml(character.character_name)}</h3>${styles ? `<div class="owned-cast__styles" aria-label="スタイル">${styles}</div>` : ""}</div>
      <div class="owned-cast__meta">
        <div class="owned-cast__status-row"><span class="owned-cast__visibility">${escapeHtml(visibilityLabel(character.visibility))}</span><span class="owned-cast__serial">${escapeHtml(displayId)}</span></div>
        <div class="owned-cast__links" aria-label="キャスト操作">
          <a href="./cast.html?id=${id}">${actionLabel("閲覧", "OPEN")}</a>
          <a href="./sheet.html?id=${id}">${actionLabel("シート編集", "EDIT SHEET")}</a>
          <a href="./acts.html?character=${id}">${actionLabel("参加アクト", "ACTS")}</a>
          <a href="./sheet.html?id=${id}#combos">${actionLabel("コンボ", "COMBOS")}</a>
        </div>
        <div class="owned-cast__management" aria-label="管理操作"><span class="owned-cast__management-label">管理操作 <small>MANAGEMENT</small></span><button type="button" data-duplicate="${escapeHtml(character.public_id)}">${actionLabel("複製", "DUPLICATE")}</button><button type="button" data-delete="${escapeHtml(character.public_id)}">${actionLabel("削除", "DELETE")}</button></div>
      </div>
    </article>`;
}
...
async function deleteCharacter(publicId) {
  ...
  const { error } = await supabase.from("characters").delete().eq("public_id", publicId).eq("owner_id", currentUser.id);
  if (error) { alert(error.message); return; }
  await loadOwnedCharacters();
}

async function duplicateCharacter(publicId) {
  const { data: source, error } = await supabase.from("characters").select("*").eq("public_id", publicId).eq("owner_id", currentUser.id).single();
  if (error) return alert(error.message);
  const sourceId = source.id;
  const copy = { ...source };
  delete copy.id; delete copy.public_id; delete copy.created_at; delete copy.updated_at;
  copy.character_name = `${copy.character_name}（複製）`;
  copy.visibility = "private";
  const { data: created, error: createError } = await supabase.from("characters").insert(copy).select("id, public_id").single();
  ...
  for (const table of ["character_skills", "character_outfits", "character_combos"]) {
    ...
    const duplicatedRows = rows.map(row => { const item = { ...row, character_id: created.id }; delete item.id; delete item.created_at; return item; });
    const { error: insertError } = await supabase.from(table).insert(duplicatedRows);
    if (insertError) return alert(insertError.message);
  }
  window.location.href = `./sheet.html?id=${encodeURIComponent(created.public_id)}`;
}
...
function formatDate(value) { if (!value) return "—"; return new Intl.DateTimeFormat("ja-JP", { dateStyle: "medium", timeStyle: "short" }).format(new Date(value)); }
function normalizeText(value) { return String(value ?? "").normalize("NFKC").toLocaleLowerCase("ja-JP").replace(/\s+/g, " ").trim(); }
