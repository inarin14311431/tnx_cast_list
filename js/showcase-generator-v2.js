...
function renderPublicCharacters() {
  ...
  elements.publicGrid.innerHTML = filtered.map(character => {
    const selectedIndex = selectedCasts.findIndex(item => !item.manual && item.character.id === character.id);
...
function handleLibraryClick(event) {
  ...
  else if (selectedCasts.length < MAX_CASTS) selectedCasts.push({ manual: false, character, description: character.summary ?? "", quote: "" });
...
function addManualCast() {
  if (selectedCasts.length >= MAX_CASTS) {
    setGeneratorStatus("キャストは最大6名までです。", "error");
    return;
  }
...
function renderSelections() {
  ...
  if (!selectedCasts.length) {
    elements.selectedCasts.innerHTML = `<p class="empty-state">公開キャストを選択するか、「手動追加」からキャストを入力してください。</p>`;
    return;
  }
...
function createManualSelection(item, index) {
  const character = item.character;
  return `
    <article class="selected-cast selected-cast--manual" data-selected-index="${index}">
...
function createOutputCastCard(item, index) {
  ...
  const displayId = item.manual ? "ScanFailed" : obfuscatePublicId(character.public_id);
  const publicUrl = item.manual ? "" : new URL(`./cast.html?id=${encodeURIComponent(character.public_id)}`, location.href).href;
  const link = item.manual
    ? `<span class="cast-card__link cast-card__link--disabled">CAST DATABASE // SCAN FAILED</span>`
...
async function publishShowcase() {
  try {
    ...
    if (!selectedCasts.length) throw new Error("参加キャストを選択してください。");
    ...
    const participantIds = selectedCasts.filter(item => !item.manual && item.character.id).map(item => item.character.id);
    ...
    setGeneratorStatus(`公開処理が完了しました。登録済みキャストの参加アクト履歴にも反映しました。GitHub Pagesへの反映には少し時間がかかる場合があります。 <a href="${escapeAttribute(data.publicUrl)}" target="_blank" rel="noopener">公開ページを開く</a>`, "success", true);
  } catch (error) {
    console.error(error);
