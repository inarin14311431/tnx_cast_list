  ...
  try {
    ...
    const { data, error } = await supabase.functions.invoke(FUNCTION_NAME, {
      body: {
        ...
        rulerName: elements.rulerName.value.trim(),
        html: generatedHtml,
        participantIds: selectedCasts.map(item => item.character.id)
      }
    });
    if (error) throw new Error(await extractFunctionError(error));
    if (!data?.publicUrl) throw new Error("公開URLを取得できませんでした。");

    setGeneratorStatus(`公開処理が完了し、参加アクト履歴へ登録しました。GitHub Pagesへの反映には少し時間がかかる場合があります。 <a href="${escapeAttribute(data.publicUrl)}" target="_blank" rel="noopener">公開ページを開く</a>`, "success", true);
  } catch (error) {
    console.error(error);
