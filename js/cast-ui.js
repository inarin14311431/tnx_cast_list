...
(() => {
  ...
  function localizeAbilityLabels(){
    const labels={VALUE:['能力値','VALUE'],CONTROL:['制御値','CONTROL'],CURRENT:['現在値','CURRENT']};
    document.querySelectorAll('.ability-card__label').forEach(el=>{const hit=labels[el.textContent.trim().toUpperCase()];if(hit)setBilingual(el,...hit);});
    const names={REASON:['理性','REASON'],PASSION:['感情','PASSION'],LIFE:['生命','LIFE'],MUNDANE:['外界','MUNDANE']};
    document.querySelectorAll('.ability-card:not(.ability-card--cs) header span:last-child').forEach(el=>{const hit=names[el.textContent.trim().toUpperCase()];if(hit)setBilingual(el,...hit);});
  }
  ...
  function localizeSkillTables(){
    const headers=[['名称','NAME'],['LV','LEVEL'],['理性','REASON'],['感情','PASSION'],['生命','LIFE'],['外界','MUNDANE'],['詳細','DETAIL']];
    document.querySelectorAll('#skills-container .data-table thead tr').forEach(row=>[...row.children].forEach((cell,index)=>{if(headers[index])setBilingual(cell,...headers[index]);}));
  }
  function localizeOutfitTables(){
    const labels={NAME:['名称','NAME'],PURCHASE:['購入','PURCHASE'],EXP:['常備化','EXP'],SLOT:['部位','SLOT'],RANGE:['射程','RANGE'],ATTACK:['攻撃','ATTACK'],DEFENSE:['防御','DEFENSE'],DESCRIPTION:['解説','DESCRIPTION']};
    document.querySelectorAll('#outfit-container th').forEach(cell=>{const hit=labels[cell.textContent.trim().toUpperCase()];if(hit)setBilingual(cell,...hit);});
  }
  ...
  function localizeProfileLabels(){
    const labels={AGE:['年齢','AGE'],GENDER:['性別','GENDER'],HEIGHT:['身長','HEIGHT'],WEIGHT:['体重','WEIGHT'],EYES:['瞳','EYES'],HAIR:['髪','HAIR'],SKIN:['肌','SKIN'],ORIGIN:['出自','ORIGIN'],EXPERIENCE:['経験','EXPERIENCE'],ENCOUNTER:['邂逅','ENCOUNTER']};
    document.querySelectorAll('#personal-data dt, #life-path dt').forEach(el=>{const hit=labels[el.textContent.trim().toUpperCase()];if(hit)setBilingual(el,...hit);});
  }
