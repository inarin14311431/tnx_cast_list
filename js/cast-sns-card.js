...
function createCardSvg(cast, theme, image) {
  ...
  const styleChips = styles.map((style, index) => `<rect x="${530 + index * 200}" y="263" width="${index === 0 ? 187 : index === 1 ? 158 : 140}" height="38" rx="4" fill="${theme.surface}" stroke="${index === 0 ? theme.accent : theme.accent2}"/><text x="${530 + index * 200 + (index === 0 ? 187 : index === 1 ? 158 : 140) / 2}" y="288" text-anchor="middle" fill="${index === 0 ? theme.accent : theme.accent2}" font-family="sans-serif" font-size="18">${esc(style.name)} ${esc(style.mark || "")}</text>`).join("");
