...
function createCardSvg(cast, theme, image) {
  ...
  const styleChips = styles.map((style, index) => { const width = index === 0 ? 187 : index === 1 ? 158 : 140; const left = 530 + index * 200 + (200 - width) / 2; const center = left + width / 2; return `<rect x="${left}" y="263" width="${width}" height="38" rx="4" fill="${theme.surface}" stroke="${index === 0 ? theme.accent : theme.accent2}"/><text x="${center}" y="288" text-anchor="middle" fill="${index === 0 ? theme.accent : theme.accent2}" font-family="sans-serif" font-size="18">${esc(style.name)} ${esc(style.mark || "")}</text>`; }).join("");
