...
supabase/19_act_participation_role.sql
```
...
`19_act_participation_role.sql`は、参加履歴ごとに「トーキー」「カブト」「共通」などの参加枠を保存する列を追加します。参加枠はキャスト所有者だけが更新でき、参加アクト一覧では専用バッジとして表示されます。
...
アクト紹介で選択した参加枠は、履歴のみ登録とGitHub Pages公開のどちらでも本人所有キャストの参加履歴へ保存されます。既存履歴には参加枠を自動補完せず、SQL適用後に新規登録または再登録した履歴から表示されます。
