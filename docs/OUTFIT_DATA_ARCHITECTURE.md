...
- 旧形式との互換性のため、論理モデルの入力境界では旧キーを受け付ける項目がある。
- 互換入力が存在する場合も、読み取り優先順位は項目ごとに固定し、暗黙に入れ替えない。
- 正規値とは異なる入力形式は「互換読み取り」として扱い、新規編集コードから無条件に二重書込みしない。
...
DBの旧トップレベル `character_outfits.electronic_control` 列は `39_drop_legacy_electronic_control.sql` で廃止する。旧形式の互換性はDB列ではなく、取込・正規化境界でトップレベル `electronic_control` キーを受け付けることで維持する。
...
2. JSON側が空の場合のみ、旧形式の入力オブジェクトにトップレベル `electronic_control` があればフォールバックとして読む。
...
3. DBの旧トップレベル列を廃止する前に、旧列だけに値があるデータを正規JSONへ退避する。
4. 正規JSONと旧列の両方に異なる非空値が存在する場合は、マイグレーションを中断し、自動DROPしない。

## 読み取り境界

PC閲覧・モバイル閲覧・キャラクターシート倉庫への転記は、生のDB行を直接解釈せず、`normalizeOutfitForView()` を通した論理モデルを使用する。

差分比較も同じ論理値の優先順位に従うこと。特に電制について、トップレベルキーとJSONを別項目として比較してはならない。
