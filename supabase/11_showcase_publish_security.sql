...
-- Prevent one publisher from overwriting an act showcase slug owned by another
-- publisher. The Edge Function performs the same check before writing to GitHub;
-- this database check closes the race window between that check and history save.
...
begin
  ...
  if found then
    if v_existing_publisher is distinct from p_published_by then
